          ...
          <div class="card-body">
            <v-row>
              <v-col>
                ...
                <router-link :to="{name:'createPermission'}"><v-icon>mdi-pencil</v-icon>Create Permission</router-link>
              </v-col>
            ...
            <table class="table">
              ...
              <tbody>
                ...
                <input v-model="selected" class="form-check-input" type="checkbox" :value="Permission.id" @change="select">
                  ...
                  <td>
                    <router-link :to="{name:'userPermission',params:{name:Permission.name}}"
  ...
  methods: {
    ...
    deleteChecked() {
      ...
      if (confirm('Do you really want to delete this item')) {
        ...
        this.axios
          .post(`/api/Permission/delete-checked`, app)
          .then((res) => {
    ...
    deletePermission(Permission) {
      if (confirm(`Delete ${Permission.title}`)) {
        this.axios
          .get(`/api/Permission/delete/${Permission.id}`)
          .then((res) => {
            this.GetAllPermission.data = this.GetAllPermission.data.filter((obj) => {
